...
The Make CLI (`@makehq/cli`) exposes a subset of Make MCP tools as command-line subcommands. When an AI agent has shell access, the CLI can be an alternative to the Make MCP server for those supported operations — it uses local shell invocation and avoids the hosted MCP server / MCP tool-calling layer, which can reduce MCP overhead; for the current CLI-to-tool mapping and any REST/curl fallbacks, see `cli-tool-invocation-mapping.md`.

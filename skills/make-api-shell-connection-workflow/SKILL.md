...
17. Do not ask the user to paste raw OAuth secrets, API keys, or passwords into chat. Use a credential request whenever a new connection must be created — but check `license.credentialRequests` on the organization first: it is an Enterprise/Partner feature, and on other plans the user must be guided through creating the connection in the scenario editor instead (see [Connection Requests](./connection-requests.md), "Plan gate"). In both flows state the exact credential paste format (Bearer prefix or raw key) — never assume the user knows it.
